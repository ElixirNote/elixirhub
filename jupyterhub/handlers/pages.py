...
class HomeHandler(BaseHandler):
    ...
    async def get(self):
        ...
        if user.active:
            ...
            # FIXME Bingqi Ji: please unify hub and lab ntp time
            actived_timestamp = datetime.strptime(user.spawner.last_activity.isoformat().split(".")[0],
                                                  '%Y-%m-%dT%H:%M:%S').timestamp() * 1000
        else:
            url = url_path_join(self.hub.base_url, 'spawn', user.escaped_name)
            actived_timestamp = datetime.utcnow().timestamp() * 1000
        ...
        html = await self.render_template(
            ...
            actived_timestamp=actived_timestamp,
        )
        self.finish(html)
...
class ExtensionsHandler(BaseHandler):
    """Handle for hub extensions management"""
